...
export default function DiscoverView({ onNavigate }: DiscoverViewProps) {
  ...
  useEffect(() => {
    async function loadDiscoverData() {
      ...
      try {
        ...
        if (selectedGenre) {
          ...
          response = await tmdb.getDiscover(mediaType, { genre: selectedGenre });
        } else {
          // Standard sections
          switch (section) {
            case 'trending':
              response = await tmdb.getTrending(mediaType, 'week');
              break;
            case 'popular':
              response = await tmdb.getPopular(mediaType);
              break;
            case 'top_rated':
              response = await tmdb.getTopRated(mediaType);
              break;
            case 'now_playing':
              response = mediaType === 'movie' 
                ? await tmdb.getNowPlayingMovies() 
                : await tmdb.getOnTheAirTV();
              break;
            case 'upcoming':
              response = mediaType === 'movie' 
                ? await tmdb.getUpcomingMovies() 
                : await tmdb.getAiringTodayTV();
              break;
          }
        }
    ...
    loadDiscoverData();
  }, [mediaType, section, selectedGenre]);
